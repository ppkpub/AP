...
define('FULLODIN_JOYDID_URI_PREFIX','ppk:514499.1525/'); //Allowed the long identification prefix in request 
define('SHORTODIN_JOYDID_URI_PREFIX','ppk:bch/'); //Allowed the short identification prefix in request 

define('PTTP_DEFAULT_KEY_FILE', AP_KEY_PATH.'514499.1525.json' ); //Allowed the short identification prefix in request 
...
function processFunctionResource($str_pttp_uri,$parent_odin_path,$resource_id){
  ...
  if($function_name=='metadata'){
    $array_result=array(
        ...
        "decimals"=>8,
        "max_data_length"=>220,
        "tx_explorer_url"=>"https://bch.btc.com/",
    );
    ...
    /*    
    $argv_pubkey = $argvs_chunks[0];
    if(strlen($argv_pubkey)>0){
        $array_result=array(
            "bitcoin"=>0.02973520,
        );
    }else{
        respPttpStatus4XX( '503',"Price service unavailable" );
        exit(-1);
    }*/
    ...
    if(strlen($argv_json_hex)>0){
        ...
        if( $coin_uri!== COIN_TYPE_BITCOINCASH ){
    ...
    if(strlen($argv_json_hex)>0){
        ...
        $coin_uri=COIN_TYPE_BITCOINCASH;
    ...
    if(strlen($argv_json_hex)>0){
        ...
        $array_result=array(
                'qrcode'=>'http://tool.ppkpub.org/swap/qr_bch.php?from='.removeCoinPrefix($array_tx_define['from_uri'],COIN_TYPE_BITCOINCASH).'&to='.removeCoinPrefix($array_tx_define['to_uri'],COIN_TYPE_BITCOINCASH).'&amount='.$array_tx_define['amount_satoshi'].'&asset='.removeCoinPrefix(@$array_tx_define['$asset_uri'],COIN_TYPE_BITCOINCASH).'&memo='.urlencode(@$array_tx_define['data']),
                'prompt'=>'Please scan with PPkAndroid APP, , and send transaction from the address('.friendlyLongID(removeCoinPrefix($array_tx_define['from_uri'],COIN_TYPE_BITCOINCASH)).')',
                'prompt_cn'=>'请使用PPk安卓应用扫码，从地址('.friendlyLongID(removeCoinPrefix($array_tx_define['from_uri'],COIN_TYPE_BITCOINCASH)).')发送交易',
            ) ;
    }else{
        respPttpStatus4XX( '400',"No argus" );
        exit(-1);
    ...
    if(strlen($argv_json_hex)>0){
        ...
        $from_address=removeCoinPrefix($array_tx_define['from_uri'],COIN_TYPE_BITCOINCASH); 
        $to_address=removeCoinPrefix($array_tx_define['to_uri'],COIN_TYPE_BITCOINCASH); 
           ...
           for($kk=0;$kk<count($array_resp['data']['list']);$kk++){
               ...
               for($pp=0;$pp<count($tmp_tx['outputs']);$pp++){
                   ...
                   if( $tmp_out['value'] == $amount &&  in_array($to_address,$tmp_out['addresses']) ){
                       $array_result = array('txid'=>$tmp_tx['hash']);
                       break;
                   }
               }
