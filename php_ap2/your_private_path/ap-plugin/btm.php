...
define('BASE_COIN_ODIN_URI', \PPkPub\PTAP02ASSET::COIN_TYPE_BYTOM ); //The base coin ODIN URI
...
function txOfQrCode($argv_json_hex)
{
    ...
    $address=\PPkPub\PTAP02ASSET::removeCoinPrefix($array_tx_define['to_uri'],BASE_COIN_ODIN_URI); 
    $amount=$array_tx_define['amount_satoshi'];
    $asset_id=\PPkPub\PTAP02ASSET::removeCoinPrefix(@$array_tx_define['asset_uri'],BASE_COIN_ODIN_URI); 
    ...
    if(@array_key_exists('transactions',$array_resp)){
       for($kk=0;$kk<count($array_resp['transactions']);$kk++){
           ...
           if( 
             ($tmp_tx['transaction_amount']==$amount || $tmp_tx['transaction_amount']==$amount+1 || $tmp_tx['transaction_amount']==$amount-1) 
               && strcasecmp($tmp_tx['details']['asset_id'],$asset_id)==0 
              ){
               //Matches transaction record that match the corresponding address & amount  &asset_id
               return array('code'=>0,"result_data"=>array('txid'=>$tmp_tx['id']));
               break;
           }
       }
    }
